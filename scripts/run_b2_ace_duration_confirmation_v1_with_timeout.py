#!/usr/bin/env python3
"""Outer GNU-timeout boundary for the one-call ACE duration confirmation."""
...
CONFIG_PATH = REPOSITORY_ROOT / "configs" / "b2_ace_duration_confirmation_v1.json"
RUNNER_PATH = REPOSITORY_ROOT / "scripts" / "run_b2_ace_duration_confirmation_v1.py"
WRAPPER_PATH = Path(__file__).resolve()
PYTHON_PATH = Path("/HOME/paratera_xy/pxy1289/.conda/envs/audio-prm/bin/python")
TIMEOUT_PATH = Path("/usr/bin/timeout")
PYCACHE_PREFIX = Path("/tmp/pxy1289-b2-ace-duration-confirmation-v1-disabled-pycache")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        version = subprocess.run(
            [str(TIMEOUT_PATH), "--version"], check=True, capture_output=True, text=True
        ).stdout.splitlines()
        ...
        environment.update(
            {
                "B2_ACE_CONFIRMATION_WRAPPER_PATH": str(WRAPPER_PATH),
                "B2_ACE_CONFIRMATION_WRAPPER_SHA256": _sha256_file(WRAPPER_PATH),
                "B2_ACE_CONFIRMATION_PRODUCTION_COMMAND_JSON": json.dumps(
                    production, separators=(",", ":")
                ),
                "B2_ACE_CONFIRMATION_TIMEOUT_COMMAND_JSON": json.dumps(
                    timeout, separators=(",", ":")
                ),
